...
// use a tree instead of an array to process middlewares
// my array based router is still consistently faster
...
export class Router {
	 ...
	 * 
	 * Traverse the router tree with a request and call all matching middlewares in the appropriate order.
	 * 
	...
	requestListener(request, response, done) {
		...
		middlewares.push(...methodwares) // Join middlewares for USE and for the request method. USE middlewares goes 1st.
		
		// call middlewares
		for (i = 0; nextCalled && i < middlewares.length; i++) {
			nextCalled = false // Must call next in a middleware to continue the loop.
			
			// Go to error handling if there is an error in any middleware.
		...
		// "Throw" 404 if no response header has been sent.
		...
		if (error) {
			for (i = 0; nextCalled && i < errorwares.length; i++) {
				...
				// Force crash if error handler throws an error.
			...
			// Force crash if no error handler sends a response.
		...
		// Go to the next external middleware if applicable.
		if (done !== void(0)) done()
	}
}
